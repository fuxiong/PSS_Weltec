...
namespace PSS_Weltec.DAL
{
    public class SqlHelper
    {
        ...
        /// <param name="tableName"></param>
        /// <returns></returns>
        public static DataSet GetListByPage(String tableName, string statusName, string status, Paging paging, string order, string sort)
        {
            DataSet ds = null;
            try
            {
                ds = new DataSet();
                string sql = "select * from (select *, ROW_NUMBER() OVER(Order by" + " " + order + " " + sort + ") as RowNumber from" + " " + tableName + " as a where " + statusName + "=" + status + ") as b where RowNumber between" + " " + ((paging.GetCurrentPage() - 1) * paging.GetPageSize() + 1) + " AND" + " " + paging.GetCurrentPage() * paging.GetPageSize() + " " + "ORDER BY" + " " + order + " " + sort + "";
                int count = GetCount("select count(*) from " + tableName + " where " + statusName + "=" + status);
                paging.SetRecordCount(count);
                SqlDataAdapter da = new SqlDataAdapter(sql, myConn);
                da.Fill(ds, tableName);
                da.Dispose();
